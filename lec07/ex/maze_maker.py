...
class Maze:
    def __init__(self, tate, yoko):
        ...
        self.map = self.generate()  # Cellインスタンスが並ぶ二次元リスト
    ...
    def generate(self):
        ...
        maze_lst = [[Cell(y, x, " ", -1) for x in range(self.yoko)]
        ...
        start_goal = [(s, g)
                      for s, g in product([0, self.tate-1], [0, self.yoko-1])]
    ...
    def show_maze(self):
        color = {" ": "white", "#": "gray",
                 "S": "#FFA07A", "G": "#87CEFA", "R": "#DBFAAF"}
        root = tk.Tk()
        canv = tk.Canvas(root, width=self.yoko*50,
                         height=self.tate*50, bg="black")
        canv.pack()
        for y in range(self.tate):
            for x in range(self.yoko):
                canv.create_rectangle(
                    x*50, y*50, x*50+50, y*50+50, fill=color[self.map[y][x].state])
                canv.create_text(
                    x*50+25, y*50+25, text=self.map[y][x].dist, font=("Helvetica", 12))
        root.mainloop()
